...
raw_cleaned2 <- raw_cleaned %>%
  dplyr::mutate(dplyr::across(c(assist02a_t2, assist03a_t2, assist04a_t2,
          assist05a_t2, assist06a_t2, assist07a_t2, assist02b_t2,
          assist03b_t2, assist04b_t2, assist05b_t2, assist06b_t2,
          assist07b_t2, assist02c_t2, assist03c_t2, assist04c_t2,
          assist05c_t2, assist06c_t2, assist07c_t2, assist02d1_t2,
          assist03d1_t2, assist04d1_t2, assist05d1_t2, assist06d1_t2,
          assist07d1_t2, assist02e_t2, assist03e_t2, assist04e_t2,
          assist05e_t2, assist06e_t2, assist07e_t2, assist02f_t2,
          assist03f_t2, assist04f_t2, assist05f_t2, assist06f_t2,
          assist07f_t2, assist02g_t2, assist03g_t2, assist04g_t2,
          assist05g_t2, assist06g_t2, assist07g_t2, assist02h_t2,
          assist03h_t2, assist04h_t2, assist05h_t2, assist06h_t2,
          assist07h_t2, assist02i_t2, assist03i_t2, assist04i_t2,
          assist05i_t2, assist06i_t2, assist07i_t2),
          as.numeric)) # Create ASSIST risk scores

raw_cleaned2 %>%
    dplyr::select(assist02a_t2, assist03a_t2, assist04a_t2, assist05a_t2,
    assist06a_t2, assist07a_t2, assist02b_t2, assist03b_t2, assist04b_t2,
    assist05b_t2, assist06b_t2, assist07b_t2, assist02c_t2, assist03c_t2,
    assist04c_t2, assist05c_t2, assist06c_t2, assist07c_t2, assist02d1_t2,
    assist03d1_t2, assist04d1_t2, assist05d1_t2, assist06d1_t2, assist07d1_t2,
    assist02e_t2, assist03e_t2, assist04e_t2, assist05e_t2, assist06e_t2,
    assist07e_t2, assist02f_t2, assist03f_t2, assist04f_t2, assist05f_t2,
    assist06f_t2, assist07f_t2, assist02g_t2, assist03g_t2, assist04g_t2,
    assist05g_t2, assist06g_t2, assist07g_t2, assist02h_t2, assist03h_t2,
    assist04h_t2, assist05h_t2, assist06h_t2, assist07h_t2, assist02i_t2,
    assist03i_t2, assist04i_t2, assist05i_t2, assist06i_t2, assist07i_t2) %>%
    dplyr::rowwise() %>%
    dplyr::transmute(tabaco = sum(assist02a_t2, assist03a_t2, assist04a_t2,
            assist05a_t2, assist06a_t2, assist07a_t2)
    )
...
ds <- raw_cleaned2 %>%
  ...
  dplyr::ungroup() %>%
  dplyr::mutate(
      dplyr::across(dplyr::starts_with("ctq"), ~ dplyr::na_if(.x, 9))) %>%
  ...
  dplyr::mutate(
    ...
    ctq_total = sum(c(ctq01_t2, ctq02_t2, ctq03_t2, ctq04_t2, ctq05_t2,
            ctq06_t2))
  ) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(
  ...
  dplyr::mutate(
      dplyr::across(dplyr::starts_with("assist01"),
          \(x) dplyr::case_when(
    x == 0 | x == 9 | is.na(x) ~ 0,
    x == 1 ~ 1
  ))) %>%
  dplyr::mutate(
    ...
    pais_tentativa = dplyr::case_when(
        b06tentsu1_t2 == 1 | b13tentsu2_t2 == 1 ~ 1, TRUE ~ 0),
    pais_medicacao = dplyr::case_when(
        b03med1_t2 == 1 | b10med2_t2 == 1 ~ 1, TRUE ~ 0),
    pais_doencapsi = dplyr::case_when(
        b01famil1_t2 == 1 | b08famil2_t2 == 1 ~ 1, TRUE ~ 0),
    ...
    transtorno_psicotico = dplyr::case_when(
        mini_m11b4_t2 == 1 | mini_m13a2_t2 == 1 | mini_m13b2_t2 == 1 ~ 1,
        TRUE ~ 0),
    panico_lifetime = dplyr::case_when(mini_e06_t2 == 1 ~ 1, TRUE ~ 0),
    bdi_severo = dplyr::case_when(somabdi_t1 >= 29 ~ 1, TRUE ~ 0),
    panico_atual = dplyr::case_when(
        pancfo_t1 == 1 | pansfo_t1 == 1 ~ 1, TRUE ~ 0)
  ) %>%
  dplyr::rowwise() %>%
  dplyr::mutate(# Create ASSIST risk scores
    ...
    alucinogenos = sum(assist02h_t2 + assist03h_t2 + assist04h_t2 +
        assist05h_t2 + assist06h_t2 + assist07h_t2),
  ...
  dplyr::select(# Desfecho (prejuízo funcional)
    ...
    # uso de substancias
    tabaco, alcool, maconha, cocaina, anfetamina, inalantes, sedativos,
    ...
    fobsoa_t1, tocat_t1, teptat_t1, tagat_t1, panico_lifetime,
  ...
  dplyr::mutate(dplyr::across(c(
    ...
    -somabdi_t1, -somasrq_t1, -hcl_total, -abuso_emocional, -abuso_fisico,
    -abuso_sexual,
...
ds <- ds %>% dplyr::mutate(dplyr::across(c(tabaco, maconha, cocaina,
                  anfetamina, inalantes, sedativos, alucinogenos, opioides,
                  alcool), ~ as.numeric(as.character(.x))))
...
drugs_df <- ds %>%
    dplyr::select(tabaco, maconha, cocaina, anfetamina, inalantes, sedativos,
        alucinogenos, opioides, alcool)

ds$any_ilicit_drug <- apply(
    ds %>% dplyr::select(maconha, cocaina, anfetamina, inalantes, sedativos,
        alucinogenos, opioides), 1, function(x){any(x == "moderate_risk")})

ds$any_ilicit_drug <- as.factor(ifelse(ds$any_ilicit_drug, "Yes", "No"))

ds$alcool_ou_tabaco <- apply(
    ds %>% dplyr::select(alcool, tabaco),
    1, function(x){any(x == "moderate_risk")})
ds$alcool_ou_tabaco <- as.factor(ifelse(ds$alcool_ou_tabaco, "Yes", "No"))

ds <- ds %>% dplyr::select(-c(maconha, cocaina,
                    anfetamina, inalantes, sedativos,
